...
class QuillDeltaToHtmlConverter extends QuillDeltaConverter {
    ...
    _renderListItem(li) {
        ...
        return (
            parts.openingTag + liElementsHtml + (li.innerList ? this._renderList(li.innerList) : '') + parts.closingTag
        )
    }
    _renderTable(table) {
        return (
    ...
    _renderTableRow(row) {
        return makeStartTag('tr') + row.cells.map((cell) => this._renderTableCell(cell)).join('') + makeEndTag('tr')
    }
